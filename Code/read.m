...
plot(xu(3),yu(3),'m*');
plot(0,0,'R*');
legend('Entorno natural UNAM','Entorno máximo NRF','Usuario 1','Usuario 2','Usuario 3','Usuario 4 (BS)');
hold off;
grid on;        
if(Bit_Canal==0)
    ...
    if(bit3Sx==0)
        M=4;  %Definimos el esquema de modulación para Texto bi (QPSK);
    else
...
if(SNR<50 || CI<7)
    warndlg('No se pudo establecer transmisión de datos','TRANSMISIÓN FALLIDA');
    cla(handles.axes12,'reset');
    cla(handles.axes13,'reset');
    cla(handles.axes14,'reset');
    set(handles.axes12,'visible', 'off');
